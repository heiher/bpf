...
$(SHARED_TARGET) : CCFLAGS+=-fPIC -fvisibility=hidden -DSHARED
$(SHARED_TARGET) : LDFLAGS+=-shared -lelf -pthread \
                            -Wl,--version-script=$(SRCDIR)/libbpf.map \
                            -Wl,-soname,libbpf.so.0

-include build.mk
CCFLAGS+=-Iinclude -Iinclude/uapi \
